  ...
  return (
    <article className="mb-4 overflow-hidden rounded-md bg-white shadow-sm">
      <div className="border-b border-black/5 px-6 pb-4 pt-6">
          ...
          <div className="relative shrink-0" ref={menuRef}>
            ...
            {menuOpen && (
              <ul className="absolute right-0 top-full z-20 mt-1 w-52 rounded-md border border-black/10 bg-white py-1 text-sm shadow-lg">
                <li>
                  <Link href="#" className="flex items-center gap-2 px-3 py-2 hover:bg-black/5">
                ...
                <li>
                  <Link href="#" className="flex items-center gap-2 px-3 py-2 text-red-600 hover:bg-black/5">
                    Delete Post
                  </Link>
                </li>
              </ul>
        ...
        <h3 className="mt-4 text-base font-semibold text-[#112032]">{post.title}</h3>
        <div
          className={`mt-4 aspect-[16/9] w-full rounded-md bg-gradient-to-br ${post.imageGradient}`}
          role="img"
          aria-label=""
        />
      </div>
        ...
        <div className="flex items-center gap-1">
          <span className="flex -space-x-1">
            {[0, 1, 2].map((i) => (
              <span
                key={i}
                className="inline-flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-gradient-to-br from-amber-200 to-rose-400 text-[10px] font-bold text-white"
              >
                {i + 1}
              </span>
            ))}
          </span>
          <span className="text-sm font-medium text-[#112032]">{post.reactSummary}</span>
        </div>
        <div className="flex gap-4 text-sm text-[#666]">
          <button type="button" className="hover:text-[#1890FF]">
            <span className="font-medium text-[#112032]">{post.commentCount}</span> Comment
          </button>
          <p>
            <span className="font-medium text-[#112032]">{post.shareCount}</span> Share
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-black/5 px-4 py-2 sm:px-6">
        ...
        >
          <svg className="text-[#112032]" xmlns="http://www.w3.org/2000/svg" width="21" height="21" fill="none" viewBox="0 0 21 21" aria-hidden>
        ...
        <button
          ...
          className="inline-flex flex-1 items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-[#112032] hover:bg-[#F8F9FB] sm:flex-none"
        >
          <svg className="text-[#112032]" xmlns="http://www.w3.org/2000/svg" width="24" height="21" fill="none" viewBox="0 0 24 21" aria-hidden>
      ...
      <div className="px-6 py-4">
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <div className="pointer-events-none absolute right-3 top-2.5 flex gap-2 text-[#112032]/50">
              <span className="pointer-events-auto">
                <button type="button" className="rounded p-1 hover:bg-black/5" aria-label="Attach">
        ...
        {post.previousCommentsCount != null && post.previousCommentsCount > 0 && (
          <button type="button" className="mt-4 text-sm font-medium text-[#666] hover:text-[#1890FF]">
        ...
        {post.comments.map((c) => (
          <div key={c.id} className="mt-4 flex gap-3">
            ...
            <div className="min-w-0 flex-1 rounded-2xl bg-[#F8F9FB] px-4 py-3">
              <Link href="#" className="font-semibold text-[#112032] hover:underline">
                {c.author}
              </Link>
              <p className="mt-1 text-sm leading-relaxed text-[#112032]">{c.body}</p>
              <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-[#666]">
                <span className="inline-flex items-center gap-1">
                  ...
                  <span className="font-medium text-[#112032]">{c.reactions}</span>
                </span>
